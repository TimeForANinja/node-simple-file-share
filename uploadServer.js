// Requirements: npm i formidable@latest
...
const upload = (req, res) => {
	...
	form.parse(req, function (err, fields, files) {
		...
		for(const file of files.filetoupload) {
			...
			while (files.includes(PATH.basename(newpath))) {
				parts = PATH.parse(newpath);
...
const home = (req, res) => {
	...
	for (const file of fetchFiles(dwPath, './')) {
		resp_strg += `<tr onclick=\'window.location = ${JSON.stringify(`${PATH.sep}get${PATH.sep}${file.loc.relative}`)}\'>`;
